    ...
    public class TreeNode
    {
        ...
        public void AddNode(TreeNode node, ref TreeNode linkFromParent)
        {
            ...
            if (node.Value < this.Value)
            {
                ...
                else LeftChild.AddNode(node, ref LeftChild);
            }
            else
            {
                if (RightChild == null)
                ...
                else RightChild.AddNode(node, ref RightChild);
            }
        ...
        // Rebalance the subtree at this node.
        private void RebalanceSubtree(TreeNode root, ref TreeNode linkFromParent)
        {
            // Get the child subtree heights.
            int leftHeight = (LeftChild == null) ? 0 : LeftChild.Height;
            int rightHeight = (RightChild == null) ? 0 : RightChild.Height;

            int balaceFactor = leftHeight - rightHeight;
            Debug.Assert(((balaceFactor >= -2) && (balaceFactor <= 2)),
                "Invalid balance factor value " + balaceFactor);
            ...
            else if (balaceFactor == 2)
            {
                // Left heavy.
                // See which grandchild is heavy.
                int leftChildHeight = (LeftChild.LeftChild == null) ? 0 : LeftChild.LeftChild.Height;
                int rightChildHeight = (LeftChild.RightChild == null) ? 0 : LeftChild.RightChild.Height;

                int childBalanceFactor = leftChildHeight - rightChildHeight;
                Debug.Assert(((childBalanceFactor >= -1) && (childBalanceFactor <= 1)),
                    "Invalid child balance factor value " + childBalanceFactor);
                Debug.Assert((childBalanceFactor != 0),
                    "Invalid child balance factor value " + childBalanceFactor);
                ...
                if (childBalanceFactor == +1)
                {
                    ...
                    RotateRight(this, ref linkFromParent);
                    return;
                }
                else
                {
                    ...
                    RotateLeft(LeftChild, ref LeftChild);
                    RotateRight(this, ref linkFromParent);
                    return;
                }
            }
            else
            {
                // Right heavy.
                // See which grandchild is heavy.
                int leftChildHeight = (RightChild.LeftChild == null) ? 0 : RightChild.LeftChild.Height;
                int rightChildHeight = (RightChild.RightChild == null) ? 0 : RightChild.RightChild.Height;
                ...
                if (childBalanceFactor == -1)
                {
                    ...
                    RotateLeft(this, ref linkFromParent);
                    return;
                }
                else
                {
                    ...
                    RotateRight(RightChild, ref RightChild);
                    RotateLeft(this, ref linkFromParent);
                    return;
                }
            }
        ...
        private void RotateRight(TreeNode B, ref TreeNode linkFromParent)
        {
            TreeNode A = B.LeftChild;
            TreeNode A2 = A.RightChild;
            linkFromParent = A;
            B.LeftChild = A2;
            A.RightChild = B;

            // Update the subtree heights.
            // Do B first because it's in A's subtree.
            B.UpdateHeight();
            A.UpdateHeight();
        }

        // Perform a left rotation.
        private void RotateLeft(TreeNode A, ref TreeNode linkFromParent)
        {
            // Rearrange the nodes.
            TreeNode B = A.RightChild;
            TreeNode B1 = B.LeftChild;
            linkFromParent = B;
            B.LeftChild = A;
            A.RightChild = B1;

            // Update the subtree heights.
            // Do A first because it's in B's subtree.
            A.UpdateHeight();
            B.UpdateHeight();
        }

        // Use a node's children to recalculate its height.
